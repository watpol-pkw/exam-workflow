    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (res.success) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'พบข้อมูลข้อสอบ',
            html: `
              <div class="text-left mt-4 text-gray-700 space-y-2">
                <p><strong>รหัสติดตาม:</strong> ${ex.tracking_code}</p>
                <p><strong>รายวิชา:</strong> ${ex.subject_name}</p>
                <p><strong>ระดับชั้น:</strong> ${ex.level}</p>
                <p><strong>ประเภทการสอบ:</strong> ${ex.exam_type}</p>
                <div class="mt-4 p-3 bg-blue-50 text-blue-800 rounded-lg border border-blue-200">
                  <strong>สถานะปัจจุบัน:</strong> <span class="text-lg font-bold">${ex.status}</span>
                </div>
                <p class="text-sm text-gray-500 mt-2">อัปเดตล่าสุด: ${new Date(ex.updated_at).toLocaleString('th-TH')}</p>
              </div>
            `,
            confirmButtonColor: '#2563eb'
          });
      ...
      <div className="max-w-5xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-10 bg-white flex flex-col justify-center relative bg-gradient-to-br from-blue-50 to-white">
          ...
          <div className="mt-auto pt-10 text-center">
            ...
            <p className="text-xs text-gray-400">V3.3.4 (Firebase) งานวัดผลและประเมินผล โรงเรียนภูเก็ตวิทยาลัย</p>
          </div>
        </div>
    ...
    Swal.fire({
      ...
      html: `
        <div class="space-y-4 text-left p-2">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700">จำนวนข้อปรนัย</label>
            ...
            <tbody>
              {receivedExams.length > 0 ? receivedExams.map((exam, i) => (
                <tr key={exam.id || i} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">{exam.objective_count}</td>
